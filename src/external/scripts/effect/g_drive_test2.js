...
if (!credentials || !credentials.refresh_token) {
  await authWithWeb()
} else if (!g_oauth.credentials) {
  g_oauth.setCredentials(credentials)
  // await g_oauth.refreshAccessToken()
}

if (Date.now() > g_oauth.credentials.expiry_date)
  try {
    await g_oauth.refreshAccessToken().catch(function () {
...
function getAuthUrl(scope) {
  ...
  const authUrl = OAuthClient.generateAuthUrl({ access_type: 'offline', scope, prompt: 'consent' })
...
async function authWithWeb() {
  ...
  await expect('oauth').then(async function (serialized) {
    ...
    return g_oauth.getToken(data.code).then(function ({ tokens }) {
      credentials = tokens
      g_oauth.setCredentials(credential)
      return setGdr({ substitute: 'g_credentials', value: neo.toMatrix(credentials) })
    })
  })
}
